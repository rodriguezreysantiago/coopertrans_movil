...
- **Plataformas activas**: Windows desktop (admin desde la oficina), Android (choferes) e iOS (config 2026-05-06: bundle `com.coopertrans.movil`, deployment target 16.0, permisos cámara/fotos en `Info.plist`, pods instalados — listo para primer `flutter run` en simulador; falta `DEVELOPMENT_TEAM` + provisioning para device real / TestFlight). Web compila pero los reportes Excel se desactivan ahí (toca `dart:io`).
