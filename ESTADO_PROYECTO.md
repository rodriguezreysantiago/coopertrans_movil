...
## 19. Sesión 2026-06-10 — Revisión total + hardening de monitoreo

> Entre la sección 18 (2026-05-22) y esta, las sesiones se registraron en
> `PENDIENTES.md` (log cronológico) y en la memoria de Drive — este doc no se
> actualizó en ese período. Hitos de ese gap: App Store iOS LIVE (2026-05-28),
> macOS enviado/rechazado/fixeado, vigilador v3 vivo (2026-06-07), update
> in-app Windows funcionando (1.2.3+), serie 1.2.x con build derivado.

Pedido: "revisión total del proyecto, nada de memoria". Auditoría desde cero con
5 agentes (app / functions / bot+cachatore / seguridad / infra) + verificación
automática completa. **Resultado: sano** — sin hallazgos críticos, sin secretos
commiteados (verificado con `git check-ignore`/`git ls-files`), rules con
default deny, suites todas verdes. Los hallazgos reales se ejecutaron como 5
tareas, completadas y deployadas en la misma sesión:

### 19.1 Alerta TELEGRAM fuera de banda — bot caído/QR (`a7caf02`, DEPLOYADA)
El aviso de "bot caído" viajaba por el propio bot (BOT_EVENTOS → resumen 8 AM
vía COLA_WHATSAPP) y el caso "bot vivo con sesión WhatsApp rota" (QR pedido)
era invisible (el heartbeat sigue latiendo). Ahora `botHealthWatchdog` (CF cada
15 min, corre en GCP independiente de la PC dedicada) avisa al Telegram de
Santiago (bot `@Coopertrans_Bot`):
- `bot_caido`: heartbeat stale >10 min · `whatsapp_roto`: AUTH_PENDIENTE /
  AUTH_FALLO inmediato; estado ≠ LISTO sostenido >30 min (cubre DESCONECTADO
  sin reconexión y "clavado en INICIANDO", sin falsa alarma por blips).
- Ciclo de incidente con anti-spam (re-aviso 60 min → cada 6 h) + 🟢 de
  recuperación con duración. Estado en `BOT_HEALTH/main.alertaExterna`.
- Lógica pura en `functions/src/bot_alerta_externa.ts` (31 tests). Secrets
  `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` (Secret Manager). Si el POST a
  Telegram falla, el próximo tick reintenta solo. Procedimiento en RUNBOOK
  ("Bot WhatsApp no envía mensajes → Cómo te enterás").
- Validado: canal E2E (mensaje de prueba recibido) + tick post-deploy 200 OK.

### 19.2 Cachatore — salud escalada por WhatsApp (`94e088a`, vivo por auto-update)
Tres puntos ciegos cerrados en `vigia.py` / `nube.py` / `choferes.py`:
- **Cloudflare bloqueando el login del scanner**: tras ~12 fallos seguidos
  (~15-20 min) con choferes esperando → WhatsApp a Santiago (key M5
  `mantenimientoBot`, fallback 35244439), re-aviso ~cada hora, ✅ al recuperar.
  TTL 60 min (origen `cachatore_salud`): fuera de horario hábil los avisos
  viejos expiran sin enviarse y el último llega fresco a la apertura (diseño
  deliberado, ver commit).
- **claves.json roto/faltante**: `_cargar_claves` tolerante (antes el error
  subía disfrazado de "error releyendo Firestore") + señal "hay objetivos
  cargados pero 0 en vigilancia" sostenida ~1 min → aviso. Se recupera solo
  (sincronizar reintenta cada 8 s).
- **Workers huérfanos**: visibilidad (purga + contador + log ERROR si ≥8
  vivos). NO se migró a ThreadPoolExecutor (tampoco mata threads; el daño ya
  estaba acotado por el timeout HTTP 20 s de `iturnos.IturnosClient`).
- El latido publica `salud` {scanner_ok, fallos_scanner, hilos_huerfanos,
  sin_vigilar} en `CACHATORE_ESTADO/bot` — la UI hoy no lo muestra (queda
  disponible para diagnóstico/futuro).

### 19.3 Functions — retry/locks/imports (`f1207b0`, TODAS deployadas)
- `fetchConReintentos` en `comun.ts` (reintenta red caída y 5xx, NO 4xx) en
  los 4 pollers que no lo tenían: volvoAlertas, volvoScores (3 intentos),
  sitrackPosicion/Eventos (2, calibrado al timeoutSeconds de cada function).
  `volvoProxy` sin retry a propósito (interactivo, mejor fallar rápido).
- `liberarLockConReintentos` en los 4 `finally` de `resumenes_diarios.ts` —
  antes un delete fallado transient silenciaba el resumen del día entero.
- `hashId` movido a `comun.ts` y los 8 imports internos `from "./index"`
  migrados a `"./comun"` (sitrack, resumenes_diarios, mantenimiento,
  historico_ibuttons, jornadas_v3_batch, jornada_historico,
  paradas_reportadas, zonas_descarga). La dependencia circular index↔módulo
  toleraba el orden de carga CJS de casualidad. `lib/index` sigue exportando
  todo vía `export *` (tests intactos).

### 19.4 Flutter housekeeping (`0d0f681`)
- 11 colecciones tipeadas a mano → `AppCollections` (constantes nuevas
  documentadas: `botHealth`, `botControl`, `stats`; bonus REVISIONES).
- 2 `TextEditingController` de dialog/sheet sin dispose (leak por apertura):
  `reportes_discrepancia._revisar` y `admin_destinatarios._elegir`.
- 3 catch vacíos deliberados documentados con su motivo.
- **Falsos positivos del audit verificados y descartados** (sin cambio): los
  mounted checks señalados ya estaban, `CommandPalette.show` no tiene async
  gap, `obtenerRuta` nunca lanza (catch interno → null), `bump_version.ps1`
  ya escribía UTF-8 correcto, y el keystore de la PC casa resuelve bien.

### 19.5 Higiene (`d23e102`)
- `PENDIENTES.md`: sección **🎯 VIGENTE** consolidada al tope (lo único a
  mirar; el log cronológico queda como historial).
- `scripts/limpiar_dist.ps1` nuevo (retención últimas 3 + copia estable;
  corrido: 38 instaladores / 858 MB liberados — todos siguen en GitHub
  Releases).
- `release_android.ps1`: mensaje de ayuda sin path hardcodeado de la PC
  oficina (el keystore es por-PC: oficina=Colo Logistica, casa=santi).

### 19.6 Pendiente operativo de esta sesión
- Validar el primer episodio real de las 2 alertas nuevas (Telegram bot +
  salud cachatore) cuando ocurra. Anotado en la sección VIGENTE de
  PENDIENTES.md.
