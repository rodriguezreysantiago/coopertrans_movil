...
**Estado (2026-05-29):** ✅ **macOS ENVIADO A REVIEW de Apple App Store** 🎉. Resuelto CAPA POR CAPA: **(1)** ✅ codesign — era la tilde en `EXECUTABLE_NAME` (binario interno ASCII). **(2)** ✅ `LSApplicationCategoryType` (`public.app-category.business`) — destrabó el deployment / "Prepare Build" (en Build 49 `Archive - macOS` ✓). **(3)** ✅ Export Compliance — `ITSAppUsesNonExemptEncryption=false` en `Info.plist`: sin esto el build subía a ASC pero quedaba colgado esperando el cuestionario y nunca llegaba a "Ready to Test". **(4)** ✅ **PANTALLA NEGRA al abrir** (commit `c7c1c62`): el build de TestFlight abría en negro. Causa: `NotificationService.init()` corría sin try/catch en `main()` y `flutter_local_notifications` 21+ tira si falta el settings de la plataforma actual — el `InitializationSettings` pasaba android/iOS/linux/windows pero **NO `macOS`** → `initialize()` reventaba ANTES de `runApp()`. Fix: agregar `macOS:` (reusa el `DarwinInitializationSettings` de iOS) + blindar las inits de arranque en try/catch (helper `initSeguro`) + `firebase_options.dart` case macOS (reusa options de iOS — sin esto `Firebase.initializeApp` tiraba `UnsupportedError`). **(5)** ✅ **Support URL** (commit `1897091`): Apple exige Support URL válida y `/soporte` daba 404 → creada `public/soporte.html` + rewrite en `firebase.json` + deploy hosting (HTTP 200). **(+)** deployment target del Podfile macOS → 11.0 (commit `be9999c`): −86% warnings de build (576 `ld` de pdfium + 9 de pods <10.13); Podfile versionado. **Listing completo** (capturas 2560×1600 con DNI propio borrado, promo, descripción) + **enviado a review el 2026-05-29**. **Pendiente:** aprobación de Apple (1-3 días) → publicar (modo manual elegido); confirmar que el login demo `00000001` sigue activo (causa #1 de rechazo). NOTA: el `Session Proxy: Unable to authenticate` del export siempre fue BENIGNO (iOS lo tiene también). Historia del bloqueo del codesign (8 builds, la tilde) abajo.
