...
**Estado:** ✅ CODESIGN RESUELTO (archive + export SUCCEEDED en Builds **46, 47 y 48**, 0 errores de firma; binario `Contents/MacOS/CoopertransMovil` ASCII). **⚠️ Bloqueo actual: la ENTREGA a App Store Connect/TestFlight falla por autenticación** — `Session Proxy Provider: Unable to authenticate with App Store Connect (Error Code=1)` en los **3 builds** (NO transient; Build 47 fue a TestFlight, 48 a App Store Connect → mismo error en ambos destinos). La app de macOS **SÍ está dada de alta** en App Store Connect (metadatos cargados). iOS entrega OK con la misma cuenta → el problema es específico de la integración **Xcode Cloud ↔ App Store Connect para macOS**. **Camino recomendado: Transporter desde la Mac** — `flutter build macos --release` (codesign ya funciona local con el fix) → Xcode Organizer → Distribute App → App Store Connect → Upload (usa el Apple ID logueado en Xcode, NO el Session Proxy de la nube), o exportar `.pkg` y subir con Transporter. Alternativa (si se insiste con la nube): re-autorizar la integración App Store Connect del workflow macOS.
