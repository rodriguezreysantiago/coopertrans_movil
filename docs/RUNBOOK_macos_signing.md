...
**Estado:** ✅ CODESIGN RESUELTO (archive+export SUCCEEDED en Builds 46/47/48, 0 errores; el `.xcresult` del archive está 100% limpio — cero `errorSummaries`/`failed`). **⚠️ La ENTREGA (`Prepare Build for App Store Connect` / TestFlight) FALLA** — el error real NO está en los logs ni en el `.xcresult` "for Runner archive" (es un paso POSTERIOR de Xcode Cloud que esos artefactos no capturan). El `Session Proxy: Unable to authenticate` del export es **BENIGNO** (warning que el iOS exitoso también tiene — descartado como causa; lo confirmó Santiago: misma cuenta sube iOS). **PROBABLE CAUSA (hallada en el .xcresult, 2026-05-28):** el archive warnea `No App Category is set for target 'Runner'` → faltaba **`LSApplicationCategoryType`** en `macos/Runner/Info.plist`, key que el **Mac App Store EXIGE** para validar/subir (iOS no la requiere igual → explica que iOS suba y macOS no). **Fix aplicado:** `LSApplicationCategoryType = public.app-category.business`. **A VALIDAR en el próximo build/subida** (NO confirmado al 100% sin el log del paso de deployment). Si persiste: conseguir el log del paso ROJO en la UI de Xcode Cloud, o subir desde la Mac (Xcode Organizer → Distribute → App Store Connect, que usa el Apple ID local y esquiva el Session Proxy).
