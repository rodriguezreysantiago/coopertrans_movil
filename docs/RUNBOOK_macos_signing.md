...
**Estado (2026-05-28 EOD):** se resolvió CAPA POR CAPA. (1) ✅ codesign — era la tilde en `EXECUTABLE_NAME`. (2) ✅ **`LSApplicationCategoryType` destrabó el FALLO del deployment**: en **Build 49** (con la categoría) `Archive - macOS` ✓ y el paso `Pruebas internas de TestFlight` **YA NO FALLA** (en 46/47/48 fallaba). (3) **Nuevo síntoma**: el paso quedó **colgado ~12h "en curso"** → muy probablemente **Export Compliance pendiente**: el build SUBE a App Store Connect pero, al faltar `ITSAppUsesNonExemptEncryption`, queda esperando el cuestionario de cumplimiento de exportación → nunca pasa a "Ready to Test" → el paso de Xcode Cloud espera eterno. **Fix agregado**: `ITSAppUsesNonExemptEncryption=false` en `macos/Runner/Info.plist` (exenta, solo HTTPS; mismo valor que iOS) → futuros builds no quedan esperando. **Para el Build 49 (YA subido):** App Store Connect → TestFlight → build macOS → responder "Cumplimiento de exportación" (exento) + cancelar el paso colgado (el build ya está en ASC, cancelar no lo borra). NOTA: el `Session Proxy: Unable to authenticate` del export siempre fue BENIGNO (iOS lo tiene también). Historia del bloqueo del codesign (8 builds, la tilde) abajo.
