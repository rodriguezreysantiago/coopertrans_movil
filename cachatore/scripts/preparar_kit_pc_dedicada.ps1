...
# =============================================================================
#
# Análogo al `whatsapp-bot/scripts/preparar_kit_pc_dedicada.ps1`. Genera
...
#   ├── claves.json             ← clave común iTurnos (snapshot al momento
...
# Repo root (un nivel arriba de cachatore/, dos niveles arriba de scripts/)
# Join-Path en PowerShell 5.1 solo acepta 2 args; encadenamos.
...
$faltantes = @()
if (-not (Test-Path $srcClaves))     { $faltantes += 'cachatore\claves.json (clave común iTurnos)' }
...
if ($faltantes.Count -gt 0) {
    ...
    Write-Host ""
    Write-Host "Restauralos primero (ver docs/SETUP_PC_DEDICADA_BOT.md) y reintentá." -ForegroundColor Yellow
    exit 1
}
...
#   1. Verifica/instala Python 3.11+ y Git (vía winget).
#   2. Clona el repo en C:\coopertrans_movil si no existe.
#   3. Copia los secrets del kit al repo (claves.json + serviceAccountKey.json).
#   4. Crea venv + instala requirements.
#   5. Registra el servicio NSSM `cachatore-vigia` (Auto + DelayedAutoStart).
#   6. (Opcional) Instala monitor de logs + iconos de escritorio.
#
# Asume que el bot YA está instalado o se va a instalar aparte (ambos
...
if (-not $pyOk) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "winget no pudo instalar Python. Instalalo manual de python.org y reintentá." }
...
if (Test-Path (Join-Path $RepoPath '.git')) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "git clone falló" }
    Ok "Repo clonado"
}
...
& $instInterno
if ($LASTEXITCODE -ne 0) { Fail "instalar_todo_cachatore.ps1 falló (exit $LASTEXITCODE)." }
...
if (-not $SkipMonitorLogs) {
    ...
    if (Test-Path $mon) {
        & $mon
        if ($LASTEXITCODE -eq 0) { Ok "Monitor de logs instalado" } else { Warn "Monitor falló (no crítico)" }
    } else {
...
Write-Host "═══════════════════════════════════════════════════════"
Write-Host ""
Write-Host "Verificá con:"
Write-Host "  Get-Service cachatore-vigia"
Write-Host ""
Write-Host "Logs en vivo (si instalaste el monitor):"
Write-Host "  cierralo y abrilo de nuevo, va a aparecer en una ventana aparte"
Write-Host ""
'@
Set-Content -Path (Join-Path $DestinoPath 'instalar_todo.ps1') -Value $instalador -Encoding UTF8
...
================================================================
"@
Set-Content -Path (Join-Path $DestinoPath 'LEEME.txt') -Value $leeme -Encoding UTF8
