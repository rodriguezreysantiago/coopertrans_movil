...
//     - auth.ts              (login + passwords + roles + rename DNI + helpers)
...
// ────────────────────────────────────────────────────────────────────
// comun.ts va PRIMERO: exporta los helpers compartidos que icm/sitrack/
// mantenimiento/resumenes importan vía "./index". auth.ts segundo
// (exporta `hashId`, idem). Re-exportarlos antes evita sorpresas de
// orden de carga.
export * from "./comun";
export * from "./auth";
export * from "./audit";
...
// La parte de index que QUEDA (telemetriaSnapshotScheduled) usa
// fetchWithTimeout de comun + secrets/VOLVO_BASE/ACCEPT_STATUSES de volvo
// (el `export *` re-exporta hacia afuera, pero no los trae al scope local).
...
// TIPOS_PELIGROSOS_SITRACK + TIPOS_CESVI_PUROS movidos a comun.ts
// (split 2026-05-19).
